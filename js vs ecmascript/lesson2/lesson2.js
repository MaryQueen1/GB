...
class BankAccount {
  static bankName = "GBank"; // то, что есть у всех
  // свойство для объекта, а не для класса
  ...
  deposit(amount) {
    ...
    this.checkPennis(amount);
    if (amount <= 0) {
      throw new Error("Wrong deposit amount, must be a positiv number");
    }
    this.balance += amount;
  }
  ...
  checkPennis(amount) {
    ...
    if (arr[1]?.length > 2) {
      // не будет выполнять последовательность, если значени до оператора было null или underfined
